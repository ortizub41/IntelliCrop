...
wheat_nasa_lm_output_significant_summary_nasa_positive <- wheat_nasa_lm_output_significant %>% filter(estimate > 0) %>% group_by(variable) %>% summarize(count = n(), estimate_mean = mean(estimate), estimate_median = median(estimate)) %>% arrange(desc(count))

wheat_nasa_lm_output_significant_summary_nasa_negative <- wheat_nasa_lm_output_significant %>% filter(estimate < 0) %>% group_by(variable) %>% summarize(count = n(), estimate_mean = mean(estimate), estimate_median = median(estimate)) %>% arrange(desc(count))
